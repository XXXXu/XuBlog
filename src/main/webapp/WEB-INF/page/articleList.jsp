      ...
      <div class="mainbar">
      	<c:forEach items="${requestScope.pageInfo.list}" var="article">
      		<div class="article mark">
      			<a class="show_article" articleid="${article.articleId}">
	      			<h3>${article.title}</h3>
	      			<p>${article.summary}</p>
	      		</a>
	      		<p>
	      			分类：
	      			<a class="getArticlesByType" articleTypeId="${article.articleType.articleTypeId}" >
	      				<strong>${article.articleType.name}</strong>
      				</a>
      				<span>&nbsp;•&nbsp;</span>
        ...
        $(function() {
        			...
        			success: function(data) {
        				...
        				$.each(articles, function(index, item) {
	        				...
	        				$("<div></div>").addClass("article mark")
	        								.append( $("<a class='show_article'></a>").attr("articleId", item.articleId)
	        								.append($("<h3></h3>").append(item.title))
	        								.append($("<p></p>").append(item.summary)) )
	        								.append( $("<p></p>").append("分类：")
	        								.append($("<a class='getArticlesByType'></a>").attr("articleTypeId", item.articleType.articleTypeId)
	        								.append($("<strong></strong>").append(item.articleType.name)))
        	...
        	$("#pageInfo").on("click", ".butons a", function() {
        		...
        		if("articleList" == arrUrlPath[3] || undefined == arrUrlPath[3]) {
        			var url = "${base}/article/getArticles?pageNum="+$(this).html();
        			...
        			var url = "${base}/article/pageChangeByType?pageNum="+$(this).html()+"&articleTypeId="+paramValue[1];
        	...
        	$(".mainbar").on("click", ".show_article", function() {
        		window.location.href="${base}/article/showArticle?articleId="+$(this).attr("articleId");
        	}); 
        	//删除
